...
class RougeService:
    ...
    def _tokenize(self, text: str) -> List[str]:
        """ÅØ½ºÆ®¸¦ ÅäÅ«È­ÇÕ´Ï´Ù."""
        # ÇÑ±Û, ¿µ¹®, ¼ýÀÚ¸¸ ÃßÃâÇÏ°í ¼Ò¹®ÀÚ·Î º¯È¯
        text = re.sub(r'[^\w\s°¡-ÆR]', ' ', text.lower())
        tokens = text.split()
    ...
    def _calculate_rouge_l(self, reference_tokens: List[str], summary_tokens: List[str]) -> Dict[str, float]:
        """ROUGE-L (Longest Common Subsequence) Á¡¼ö¸¦ °è»êÇÕ´Ï´Ù."""
        def lcs_length(x, y):
            m, n = len(x), len(y)
    ...
    def calculate_rouge_scores(self, reference_text: str, summary_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"ROUGE °è»ê Áß ¿À·ù: {e}")
            return {
