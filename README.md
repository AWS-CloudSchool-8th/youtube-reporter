...
- **색상 팔레트**: 다양한 테마별 색상 조합 자동 적용
- **지능형 배치**: 컨텍스트에 따라 최적의 위치에 자동 배치
- **품질 최적화**: 중복 제거, 밸런스 조정, 가독성 개선
...
### 3. Visual Agent
- **역할**: 스마트 시각화 생성
- **기술**: 컨텍스트 분석 + 시각화 생성 + 자동 최적화
- **출력**: Chart.js, Mermaid, HTML 시각화
- **개선된 기능**:
  - 컨텍스트 기반 시각화 기회 자동 탐지
  - 다양한 색상 팔레트 지원
  - 시각화 타입별 최적화
  - 자동 위치 선정 알고리즘
  - 시각화 품질 개선 및 최적화
...
| 타입 | 라이브러리 | 용도 | 지원 형식 |
|------|------------|------|------|
| **차트** | Chart.js | 데이터 비교, 트렌드, 비율 | bar, line, pie, doughnut, radar, scatter |
| **다이어그램** | Mermaid | 프로세스, 플로우, 관계도 | flowchart, timeline, mindmap, gantt |  
| **테이블** | HTML | 정확한 데이터 비교 | 정렬, 필터링, 하이라이트 기능 |
| **고급** | D3.js | 복잡한 데이터 시각화 | 고급 인터랙티브 시각화 |

### 색상 팔레트

| 팔레트 이름 | 용도 | 사용 예시 |
|------|------------|------|
| **기본** | 일반적인 데이터 표현 | 다양한 범주의 데이터 비교 |
| **순차적** | 순차적 값이나 등급 표현 | 시간에 따른 변화, 등급 표현 |
| **대비적** | 대비되는 값 표현 | 양극단 값 비교, 차이 강조 |
| **범주형** | 분류 데이터 표현 | 서로 다른 범주의 데이터 구분 |
| **강조** | 특정 값 강조 | 중요한 데이터 포인트 강조 |
